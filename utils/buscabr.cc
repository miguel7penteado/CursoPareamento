...
std::string trim_reduce(const std::string &str)
{
    ...
    size_t begin = str.find_first_not_of(token);
    ...
    // find first token
    size_t start = toRet.find_first_of(token);
...
void replace_word_starting_from(std::string &str, const std::string &from, size_t len)
{
    size_t start = str.find(from);
    while(start != std::string::npos)
    {
...
std::string join(const std::vector<std::string>& v) {
  ...
  copy(v.begin(), v.end(), std::ostream_iterator<std::string>(res, " "));
  return res.str();
}
